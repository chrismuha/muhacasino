...
try {
  const imported = await import("../../../PRACTICE SOFTWARE SUITE/_shared/fix-electron-builder-dep0190.mjs");
  if (typeof imported.fixElectronBuilderDep0190 === "function") {
    fixElectronBuilderDep0190 = imported.fixElectronBuilderDep0190;
  }
} catch {
  // Shared patch helper is optional; continue without it.
}
...
const tempOutputDir = path.join("/private", "tmp", "bingo-electron-builder-output");
...
function isMacBuildArtifact(entryName) {
  ...
  if (/\.(dmg|zip|blockmap)$/.test(entryName)) return true;
  return false;
}
  ...
  if (process.platform === "darwin" && code === 0 && !hasCustomOutput && fs.existsSync(tempOutputDir)) {
    try {
      ...
      for (const entry of entries) {
        ...
        const destinationPath = path.join(projectOutputDir, entry);

        fs.rmSync(destinationPath, { recursive: true, force: true });
        execFileSync("cp", ["-R", sourcePath, projectOutputDir], { stdio: "ignore" });
        copied.push(entry);
      }
