// Config
const WILD_SYMBOL = "lantern";
const BONUS_SYMBOL = "bonus";
const SCATTER_SYMBOL = "scatter";
const LINK_SYMBOL = "LINK";
const SYMBOLS = [WILD_SYMBOL, "clover", "harp", "horseshoe", "rainbow", "crown", "gem"];
const SYMBOL_LABELS = {
    lantern: "TreasurePots wild",
    clover: "Lucky clover",
    harp: "Golden harp",
    horseshoe: "Golden horseshoe",
    rainbow: "Treasure rainbow",
    crown: "Royal crown",
    gem: "Emerald gem",
};
...
const PAYTABLE = {
    ...
    clover: { 3: 60, 4: 240, 5: 1200 },
    harp: { 3: 30, 4: 120, 5: 600 },
    horseshoe: { 3: 18, 4: 72, 5: 360 },
    rainbow: { 3: 12, 4: 48, 5: 240 },
    crown: { 3: 8, 4: 30, 5: 150 },
    gem: { 3: 5, 4: 20, 5: 100 },
};
...
const SAVINGS_BANKS = [
    ...
    { name: "Royal Pot", amountUSD: 100 },
];
const BONUS_WHEEL_SEGMENTS = [
    { kind: "coin", name: "Copper Coin", mark: "1×", multiplier: 1, weight: 0.35, start: 0, end: 0.35 },
    { kind: "coin", name: "Lucky Coin", mark: "2×", multiplier: 2, weight: 0.25, start: 0.35, end: 0.60 },
    { kind: "coin", name: "Ruby Coin", mark: "5×", multiplier: 5, weight: 0.18, start: 0.60, end: 0.78 },
    { kind: "coin", name: "Golden Coin", mark: "10×", multiplier: 10, weight: 0.12, start: 0.78, end: 0.90 },
...
function updateFeatureStatus() {
    ...
    if (freeSpinsRemaining > 0) {
        ...
        featureStatusEl.textContent = "Free Spins & Emerald Wheel active";
    } else {
        featureStatusEl.textContent = `Free Spins ${freeSpinsActive ? "active" : "not active"} • Emerald Wheel ${bonusActive ? "active" : "not active"}`;
    }
...
function createCell(symbol, isWinning = false) {
    ...
    if (symbol === BONUS_SYMBOL) {
        ...
        chip.innerHTML = "<strong>Emerald</strong><span>Spin Wheel</span>";
        chip.setAttribute("aria-hidden", "true");
        cell.appendChild(chip);
        cell.classList.add("jackpot-win", "bonus-win");
        cell.setAttribute("aria-label", "Emerald Wheel trigger chip");
        return cell;
    }
    if (symbol === SCATTER_SYMBOL) {
        const scatter = document.createElement("span");
        scatter.className = "scatter-chip";
        scatter.innerHTML = "<strong>SCATTER</strong><span>FREE SPINS</span>";
        cell.appendChild(scatter);
        cell.classList.add("scatter-cell");
        cell.setAttribute("aria-label", "Treasure chest scatter");
        return cell;
    }
...
function addScatterSymbols(grid, triggered) {
    if (!triggered) return { grid, winningPositions: new Set(), count: 0 };
    const displayGrid = grid.map((row) => [...row]);
    const winningPositions = new Set();
    ...
    positions.forEach(([row, col]) => {
        displayGrid[row][col] = SCATTER_SYMBOL;
        winningPositions.add(`${row},${col}`);
    });
    return { grid: displayGrid, winningPositions, count };
}
...
async function animatePotPop(popped) {
    ...
    popped.forEach((index) => document.querySelector(`.pot[data-pot="${index}"]`)?.classList.add("popping"));
