...
const BONUS_WHEEL_SEGMENTS = [
    { kind: "coin", name: "Copper Coin", mark: "10×", multiplier: 10, weight: 0.35, start: 0, end: 0.35 },
    { kind: "coin", name: "Lucky Coin", mark: "25×", multiplier: 25, weight: 0.25, start: 0.35, end: 0.60 },
    { kind: "coin", name: "Ruby Coin", mark: "50×", multiplier: 50, weight: 0.18, start: 0.60, end: 0.78 },
    { kind: "coin", name: "Golden Coin", mark: "100×", multiplier: 100, weight: 0.12, start: 0.78, end: 0.90 },
...
function renderOutcomeGrid(grid, winningPositions, jackpotWins, jackpotTeasers = []) {
    ...
    for (const jackpotWin of jackpotSymbols) {
        const position = preferredPositions.find(([r, c]) =>
            !displayPositions.has(`${r},${c}`) && displayGrid[r][c] !== BONUS_SYMBOL && !jackpotPositions.has(`${r},${c}`)
        )
            || preferredPositions.find(([r, c]) => displayGrid[r][c] !== BONUS_SYMBOL && !jackpotPositions.has(`${r},${c}`));
