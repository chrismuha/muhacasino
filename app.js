...
/* ====== UI ====== */
const gridEl = document.getElementById('track'); // updated to the new track container
...
function renderBoard() {
  // Use the new #track container instead of the old rectangular grid
  const trackEl = document.getElementById('track');
  // clear old cells
  trackEl.querySelectorAll('.cell').forEach(n => n.remove());
  ...
  const cx = W / 2, cy = H / 2;
  const a = Math.min(W, H) * 0.40; // horizontal radius
  const b = Math.min(W, H) * 0.30; // vertical radius

  // Map track index -> position on an oval path
  // index 0 at top, advance clockwise
  function posFor(i) {
  ...
  // render the 60 cells around the loop
  for (let i = 0; i < BOARD_LEN; i++) {
    const cell = document.createElement('div');
    cell.className = 'cell';

    // slide tinting by owner on slide-start cells
    const sl = SLIDES.find(s => s.start === i);
    if (sl) {
    ...
    idx.className = 'idx';
    idx.textContent = i; // hidden via CSS, handy for debugging
